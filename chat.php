					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<strong class="card-title">CHAT WITH MATCH</strong>
							</div>
							<div class="card-body">
							<?php
								
								$yrq = "SELECT * FROM `usertable` JOIN `requesttable` ON usertable.user_Id = requesttable.recipient_id WHERE recipient_status = 1 AND fk_user_id = $id ";
								$ty = mysqli_query( $con, $yrq );
								$num = mysqli_num_rows( $ty );
								
								while($dd = mysqli_fetch_array($ty))
								{
								?>
                                
								<div class="col-md-12">
									<a href="#">
								<div class="card" style="background-color:#5AB0EF ;" id="load" data-touserid="<?php echo $dd['user_Id'] ?>" data-tousername="<?php echo $dd['userFirstName'] ?>" >
							<div class="card-body" >
								<font size="4" color="#FBFBFB" ><?php echo $dd['userFirstName'] ?></font> &ensp;
								<span id="notifyme" class="bg-danger" style="color:#FFFFFF;border-radius: 10%;"></span>
							</div>
...
<script>
 ...
 $(document).on('click', '#load', function(){
  ...
  $.ajax({
   url:"insert_chat.php",
   method:"POST",
   data:{from_user_id:from_user_id,to_user_id:to_user_id, chat_message:chat_message,name:name},
 ...
 var from_user_id = <?php echo $id; ?>;
  $.ajax({
   url:"chatHistory.php",
   method:"POST",
   data:{to_user_id:to_user_id,from_user_id:from_user_id,name:name},
   success:function(data){
    $('#chat_history_'+to_user_id).html(data);
   }
  })
